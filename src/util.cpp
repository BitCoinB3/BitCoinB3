...
const char * const BITCOINB3_CONF_FILENAME = "bitcoinb3.conf";
const char * const BITCOINB3_PID_FILENAME = "bitcoinb3d.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "bitcoinb3d -foo=bar
...
#else
    const char* pszModule = "bitcoinb3";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BitcoinB3
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BitcoinB3
    // Mac: ~/Library/Application Support/BitcoinB3
    // Unix: ~/.bitcoinb3
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BitcoinB3";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BitcoinB3";
#else
    // Unix
    return pathRet / ".bitcoinb3";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", BITCOINB3_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", BITCOINB3_PID_FILENAME)));
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Bitcoin Core") == std::string::npos) {
        ...
        strYear.replace(strYear.find("2018"), sizeof("2018")-1, "2009-2018");

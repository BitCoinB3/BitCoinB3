...
###   bitcoinb3core.org and bitcoinb3.org and compares them.
...
WORKINGDIR="/tmp/bitcoinb3_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://bitcoinb3core.org"
HOST2="https://bitcoinb3.org"
BASEDIR="/bin/"
VERSIONPREFIX="bitcoinb3-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'bitcoinb3-',
...
if [ $? -ne 0 ]; then
   echo "bitcoinb3.org failed to provide signature file, but bitcoinb3core.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "bitcoinb3.org and bitcoinb3core.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the BitcoinB3 Core binary release signing key installed?"
   fi
